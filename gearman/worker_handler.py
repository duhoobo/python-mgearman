...
from gearman.protocol import GEARMAN_COMMAND_PRE_SLEEP, \
        GEARMAN_COMMAND_RESET_ABILITIES, GEARMAN_COMMAND_CAN_DO, \
        GEARMAN_COMMAND_SET_CLIENT_ID, GEARMAN_COMMAND_GRAB_JOB_UNIQ, \
        GEARMAN_COMMAND_WORK_STATUS, GEARMAN_COMMAND_WORK_COMPLETE, \
        GEARMAN_COMMAND_WORK_FAIL, GEARMAN_COMMAND_WORK_EXCEPTION, \
        GEARMAN_COMMAND_WORK_WARNING, GEARMAN_COMMAND_WORK_DATA
...
class GearmanWorkerCommandHandler(GearmanCommandHandler):
    ...
    A worker can be in the following distinct states:
        ...
        AWAITING_JOB  -> Holding worker level job lock and awaiting a server 
                         response
    ...
    def recv_job_assign_uniq(self, job_handle, task, unique, data):
        ...
        AWAITING_JOB -> EXECUTE_JOB -> SLEEP :: Always transition once we're 
                                                given a job
        """
        assert task in self._handler_abilities, \
                '%s not found in %r' % (task, self._handler_abilities)

        # After this point, we know this connection handler is holding onto the 
        # job lock so we don't need to acquire it again
        if not self.connection_manager.check_job_lock(self):
            raise InvalidWorkerState("Received a job when we weren't expecting one")

        gearman_job = self.connection_manager.create_job(self, job_handle, task,
                                                         unique, 
